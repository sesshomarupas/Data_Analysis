...
index_List = list(range(len(G1)))
alpha = 0.05

# Ausreißer identifizieren 
outliersIdentification = str(input("Möchten Sie die Ausreißer entfernen? Tippen Sie 'ja' wenn ja, tippen Sie 'nein' wenn nein: "))
if outliersIdentification == "ja":
    def outliersDetection(x):
        medianDiffx = median(x); mad = robust.mad(x)
        borderUpx = medianDiffx + 3*mad; borderDownx = medianDiffx - 3*mad
        dataWithOut = []
        for i in x:
            if i > borderUpx or i < borderDownx:
                dataWithOut.append("NaN")
            else:
                dataWithOut.append(i)
        return dataWithOut
    d1 = pd.DataFrame({"Group1/Data1_without_Outliers": outliersDetection(G1)}, index = index_List); d2 = pd.DataFrame({"Group2/Data2_without_Outliers": outliersDetection(G2)}, index = index_List); d3 = pd.DataFrame({"Group3/Data3_without_Outliers": outliersDetection(G3)}, index = index_List)
    listDataFrames = [d1, d2, d3]
    dataFrame_without_Outliers = pd.concat(listDataFrames, axis = 1)
    G1 = dataFrame_without_Outliers.iloc[:,0]; G2 = dataFrame_without_Outliers.iloc[:,1]; G3 = dataFrame_without_Outliers.iloc[:,2]
#In[]
